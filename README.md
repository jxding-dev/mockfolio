...
1. 투명 오버레이를 `public/mockups/overlays/{category}/` 아래에 넣는다 (사용자 이미지가 들어갈 영역은 alpha로 뚫는다). **배포 용량을 위해 WebP(투명 지원) 권장** — 기존 에셋은 모두 WebP다. PNG도 동작하지만 용량이 크다.
2. `public/mockups/manifest.json`에 등록한다:

```json
{ "id": "unique-id", "label": "표시 이름", "category": "표시 카테고리", "src": "mockups/overlays/app/my-mockup.webp" }
```
